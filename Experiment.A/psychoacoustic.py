...
def smr_bit_allocation(params,smr):
  ...
  ## in the situation only coding one subband, give the 2 bit a sample, adding basic information how many bits are used
  if bits_available <= 2 * FRAMES_PER_BLOCK + 6:
    sys.exit('Insufficient bits for encoding.')

  ## how many noise will incur when setting in different bit allocation
  snr = params.table.snr
  
  mnr = snr[bit_allocation[:]] - smr

  #MNR is a measure of how much the masking sound reduces the intelligibility of the desired sound (the "signal") by creating a background noise that makes the signal less audible.
  #A positive MNR indicates that the masking sound is effectively hiding the original sound, while a negative MNR suggests the original sound is still audible.
  #The goal is to have a high MNR to ensure that speech privacy is maintained in a space where sound masking is implemented

  #The key equation is this, we already know the lower mask-noise ratio, the more possible the artifacts might get explosure, therefore the higher the bit allocation, 
  #the less noise will be incurred. following code is how to find the lowest value than dispence the bits to the subband.
  while bits_available >= FRAMES_PER_BLOCK:
    ...
    ###A location of MAX band become a strategy that filter the subband which improper to add bit, although 
    ###it definitle bring the confuse to the user,to recode is a better choice.
  ...
  ##recalculated the mnr before it output, this excluded the ranking affection
  final_mnr = np.zeros_like(smr)
  for i in range(N_SUBBANDS):
    if bit_allocation[i] == 0:
        final_mnr[i] = snr[0] - smr[i]
    else:
        final_mnr[i] = snr[bit_allocation[i] - 1] - smr[i]
  #print(f"After Allocation the bit remains are: {bits_available}")
  #print(f"The Mask-Noise Ratio is: {np.where(formatted_mnr == 123456., 'MAX', formatted_mnr)}")
  #print(f"The bit allocation is: {bit_allocation}")
  
  return bit_allocation,final_mnr
...
def model1(samples, params, sfindices):
  ...
  #print(f"Current SMR on band is: {smr}\n")

  subband_bit_allocation,mnr = smr_bit_allocation(params, smr)
  return subband_bit_allocation,mnr
